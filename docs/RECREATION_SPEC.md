...
10. 🆕 **Novo (não é divergência, é feature nova).** Contexto de tela da janela ativa
    (Windows only): a cada ditado elegível a cleanup/agent, o app pode capturar um
    screenshot da janela em foco, rodar OCR local, e injetar o texto extraído no prompt
    do LLM de cleanup/agent — nunca no transcript bruto. Default ON (desvio deliberado da
    postura de privacidade-por-padrão, documentado como tal). Gating síncrono no
    hotkey-down (`shouldCaptureScreenContext()`); cache de reuso de OCR entre ditados
...
_(convenções: **invoke** = dois sentidos com retorno; **on/send** = um sentido; **push** = main→renderer sem invoke correspondente)_
...
if (useLocalWhisper) {
  if (localProvider === "nvidia") {
    processWithLocalParakeet;
  } else {
    processWithLocalWhisper;
  }
} else {
...
**Regra crítica de GPU**: Parakeet **sempre** tenta CUDA quando GPU + binário CUDA presentes — sem toggle de CPU equivalente ao `WHISPER_GPU_MODE`. Citação do próprio código: _"NVIDIA (Parakeet) transcription always runs on CUDA when the hardware and the CUDA binary are both present ... Selecting CPU only ever applies to the OpenAI/Whisper engine — NVIDIA models are never downgraded to CPU while a usable GPU is available."_
...
`isCleanup = !config.systemPrompt` → `temperature=0`, texto do usuário enviado **cru** (sem wrapper — ver nota abaixo). Com `systemPrompt` (agente) → `temperature≈0.3`, texto também cru. `max_tokens = config.maxTokens || max(4096, calculateMaxTokens(...))`. **Regra gpt-oss**: cleanup + modelo `gpt-oss` → `reasoning_effort="low"`. `applyThinkingSuppression()` (§5.7). Retry com backoff (`withRetry`, 3 tentativas). Resposta vazia → erro.

**Nota (docs/specs/prompt-template-placeholders.md)**: `wrapCleanupTranscript()` — que envolvia o texto ditado em `<transcript>...</transcript>\n\nOutput only the cleaned transcript.` antes de enviá-lo como mensagem `role: "user"` — foi **removida por completo** do código (`src/config/prompts/index.ts`, `src/config/prompts.ts` e todos os call sites em `ReasoningService.ts` e nos 5 provedores de `src/services/ai/inferenceProviders/` — `openai.ts`, `anthropic.ts`, `gemini.ts`, `local.ts`, `enterprise.ts`). O texto ditado agora vai direto como conteúdo da mensagem do usuário, sem delimitador — o papel da mensagem já marca isso como conteúdo do usuário. O contrato de saída ("Output ONLY the cleaned transcript...") foi reescrito para o final do próprio template `cleanupPrompt` (ver §5.6.3).
...
`PROMPT_KINDS = {cleanup, dictationAgent, chatAgent}` (chaves i18n `cleanupPrompt`/`fullPrompt`; `chatAgent` não tem chave i18n, usa `DEFAULT_CHAT_AGENT_PROMPT` hardcoded). `resolvePrompt(kind, opts)`: prompt customizado do usuário (`customPrompts[kind]`) `|| getDefaultPromptText(kind, uiLanguage)`, depois `applySubstitutions(template, opts)`.

**Mecanismo de placeholders posicionais (docs/specs/prompt-template-placeholders.md)** — substitui o antigo mecanismo de "append incondicional no fim do prompt". `applySubstitutions()`: primeiro substitui `{{agentName}}` (sempre, incondicional), depois calcula três blocos — `buildLanguageBlock(language)` (instrução de idioma, ou `""`), `buildDictionaryBlock(customDictionary, uiLanguage)` (sufixo do Dicionário Personalizado, ou `""`), `buildScreenContextBlock(screenContextText, uiLanguage)` (bloco de contexto de tela OCR'd — ver `docs/specs/active-window-screen-context.md` — ou `""`) — e chama `applyPromptPlaceholders(template, {languageBlock, dictionaryBlock, screenContextBlock})`, que substitui **todas** as ocorrências de `{{languages}}`, `{{user-dictionary}}` e `{{screen-ocr}}` pelo respectivo bloco (ou `""` quando vazio). **Não há append-if-missing**: um token ausente do template não recebe nada — decisão deliberada do dono do projeto (ver spec). `ResolvePromptOptions` agora inclui `screenContextText?: string | null`, usado por `getCleanupSystemPrompt()` (5º parâmetro) e pela rota `dictationAgent` de `audioManager.js` — ambas passam `screenContextText` direto para `resolvePrompt()`, sem mais append externo. `appendDictionarySuffix`/`appendScreenContextSuffix` continuam exportadas com a mesma assinatura (`prompt + block`), agora como wrappers finos sobre `buildDictionaryBlock`/`buildScreenContextBlock`. Os templates padrão `cleanupPrompt`/`fullPrompt` (`en`/`pt`) contêm os três tokens (nessa ordem), então usuários no prompt padrão não regridem; apenas um prompt customizado salvo antes dessa mudança — que não contém nenhum token — deixa de receber esses três blocos (perda aceita explicitamente pelo dono do projeto, sem migração — ver spec).

`wrapCleanupTranscript()` foi **removida** (ver §5.2.1 acima) — o texto do usuário não é mais envolvido em `<transcript>` tags; o contrato de saída agora vive no fim do próprio template `cleanupPrompt`, após os três placeholders.

`getAgentSystemPrompt` (não afetado por este mecanismo — chama `resolvePrompt("chatAgent", {agentName: null})` sem `customDictionary`/`language`/`screenContextText`, e `DEFAULT_CHAT_AGENT_PROMPT` não contém nenhum dos três tokens) monta prompt do chat do agente + instruções por tool + bloco de notas relevantes (RAG).
...
_Fim da especificação. Este documento foi gerado por pesquisa automatizada de 7 agentes independentes sobre o estado do código na branch `chore/remove-dead-cloud-code`. Onde o comportamento divergir do `CLAUDE.md` do projeto, confie neste documento e no código-fonte, não na documentação de referência antiga._
