...
**Preview de ditação**: `start-dictation-preview`, `dictation-preview-audio` (on), `dismiss/hide-dictation-preview`, `complete-dictation-preview`, `update-cleanup-preview`, `resize-transcription-preview-window`, `stop-dictation-preview`; push: `preview-text/append/hold/result/cleanup-update/hide`.
...
`npm test` = `node --test` (runner nativo, sem Jest/Mocha). 52 arquivos (42 `test/helpers/` + 10 `test/utils/`), cada um autocontido com mocks via interceptação de `Module._load`. Cobertura: wake word, roteamento de ditado, dicionário/eco, hotkeys, backends llama, VAD/streaming, GPU mode resolver, snippets, etc.
