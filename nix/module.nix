...
let
  cfg = config.programs.ektoswhispr;
in
{
  options.programs.ektoswhispr = {
    enable = lib.mkEnableOption "EktosWhispr voice dictation app with Wayland auto-paste support";
    ...
    users = lib.mkOption {
      ...
      description = ''
        ...
        ydotool group (to reach the ydotoold socket) and the uinput group (EktosWhispr's
  ...
  config = lib.mkIf cfg.enable {
    ...
    # do this, and EktosWhispr's linux-fast-paste --uinput needs /dev/uinput directly.
