---
name: ektoswhispr-api
description: Use this skill when building integrations with the EktosWhispr REST API, calling EktosWhispr endpoints, managing notes/folders/transcriptions programmatically, or connecting to the EktosWhispr MCP server. Covers authentication, all V1 endpoints, pagination, rate limits, error handling, and the remote MCP server.
---

# EktosWhispr API v1

Use this reference when making requests to the EktosWhispr REST API. All endpoints are under the V1 path and require API key authentication.
...
Generate keys from the EktosWhispr desktop app under **Settings > API Keys**. Keys start with `owk_live_` and are shown once at creation.
...
```
https://api.ektoswhispr.com/api/v1
```
...
```bash
claude mcp add ektoswhispr --transport http https://mcp.ektoswhispr.com/mcp \
...
```json
{
  "mcpServers": {
    "ektoswhispr": {
      "url": "https://mcp.ektoswhispr.com/mcp",
...
curl -H "Authorization: Bearer owk_live_YOUR_KEY" \
  "https://api.ektoswhispr.com/api/v1/notes/list?limit=10"
```
...
curl -X POST \
  ...
  https://api.ektoswhispr.com/api/v1/notes/create
```
...
curl -X POST \
  ...
  https://api.ektoswhispr.com/api/v1/notes/search
```
...
while true; do
  response=$(curl -s -H "Authorization: Bearer owk_live_YOUR_KEY" \
    "https://api.ektoswhispr.com/api/v1/notes/list?limit=100&cursor=${cursor}")
...
```bash
curl -H "Authorization: Bearer owk_live_YOUR_KEY" \
  https://api.ektoswhispr.com/api/v1/usage
```
