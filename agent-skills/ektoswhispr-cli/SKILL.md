...
description: Use this skill whenever the user wants to operate on EktosWhispr notes, folders, transcriptions, or audio from a terminal or shell. The EktosWhispr CLI (`ektoswhispr` binary, npm package `@ektoswhispr/cli`) talks to either the local desktop app or the cloud API and exposes every operation needed for managing notes, folders, transcriptions, audio, plus auth and config. Trigger this skill when the user mentions "ektoswhispr cli", running shell commands against EktosWhispr, automating note workflows, cleaning up a transcription, building agent integrations against EktosWhispr, or scripting any EktosWhispr operation — even if they don't say "CLI" explicitly.
---

# EktosWhispr CLI

Use this reference when running the `ektoswhispr` command-line tool. The CLI is a single binary that operates against either the local desktop app (via a loopback HTTP bridge) or the cloud REST API. The same command works against both backends.

## Install

```bash
npm install -g @ektoswhispr/cli
```

Requires Node.js 20 or later. Verify with `ektoswhispr --version`. If the user reports `command not found`, ensure their npm global bin is on `$PATH`.
...
| **remote** | `https://api.ektoswhispr.com/api/v1`               | Desktop is closed, or running on a different machine, or the user wants cloud-side semantics. |
...
2. The `EKTOSWHISPR_BACKEND` environment variable (`local`, `remote`, or `auto`)
3. The `backend` key in `~/.ektoswhispr/cli-config.json`
...
When the desktop app starts, it writes `{version, port, token}` to `~/.ektoswhispr/cli-bridge.json` with mode `0600`. The CLI reads it automatically. If the file is missing or stale, local is treated as unavailable.
...
```bash
ektoswhispr auth login    # prompts for the key, stores it 0600 in ~/.ektoswhispr/cli-config.json
ektoswhispr auth status   # confirm it works
ektoswhispr auth logout   # clear it
```
...
Noun-verb syntax: `ektoswhispr <noun> <verb>`. Same convention as `gh`, `kubectl`, `aws`, `stripe`.

### Notes

```bash
ektoswhispr notes list [--folder <id>] [--limit N] [--format json|table]
ektoswhispr notes get <id> [--format json|markdown]
ektoswhispr notes create --content <text> | --content-file <path>
                        [--title <t>] [--folder <id>]
ektoswhispr notes update <id> [--content <t>] [--folder <id>] [--title <t>]
ektoswhispr notes delete <id> [--dry-run] [--format json]
ektoswhispr notes search <query> [--limit N] [--format json|table]
```

### Folders

```bash
ektoswhispr folders list [--format json|table]
ektoswhispr folders create --name <name> [--sort-order <n>]
```
...
```bash
ektoswhispr transcriptions list [--limit N] [--format json|table]
ektoswhispr transcriptions get <id> [--format json|text]
ektoswhispr transcriptions delete <id> [--dry-run] [--format json]
```
...
```bash
ektoswhispr audio delete <transcription-id> [--format json]
```
...
```bash
ektoswhispr auth login [--api-key <key>]   # prompts on stdin if --api-key omitted
ektoswhispr auth status [--format json]
ektoswhispr auth logout
```

`auth status` reads the stored config and reports whether a key is configured — it does **not** make a network call. To verify the key actually works, use `ektoswhispr doctor`.

### Config

```bash
ektoswhispr config get [--format json]
ektoswhispr config set backend auto|local|remote
ektoswhispr config set api-base https://api.ektoswhispr.com
```

Only `backend` and `api-base` are settable via `config set`. The API key is managed through `auth login`/`auth logout`. `api-base` is overridable for self-hosted or staging deployments (default: production cloud). The `EKTOSWHISPR_API_BASE` env var also overrides it for a single invocation.

### Doctor

```bash
ektoswhispr doctor [--format json]
```
...
```bash
ektoswhispr --version    # or: ektoswhispr version
```
...
```bash
ektoswhispr notes list --limit 100 --format json | \
  jq -r '.[] | select(.title | contains("draft")) | .id' | \
  while read id; do
    ektoswhispr notes delete "$id"
  done
```
...
```bash
ektoswhispr notes search "quarterly budget" --format json | jq '.[].id'
```
...
| ------------------------------- | --------------------------------------- | ------------------------------------------------ |
| `~/.ektoswhispr/cli-bridge.json` | The desktop app at startup              | `{version, port, token}` for the loopback bridge |
| `~/.ektoswhispr/cli-config.json` | The CLI's `auth login` and `config set` | `{backend, apiBase, apiKey}`                     |
...
| ----------------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------- |
| `Backend unreachable` (exit 2) on every command | Desktop closed and no API key                                 | Start desktop, or `ektoswhispr auth login`          |
...
| Config file readable by other users             | File created or edited outside the CLI                        | `chmod 0600 ~/.ektoswhispr/cli-config.json`         |
