...
class DatabaseManager {
  ...
  initDatabase() {
    try {
      ...
      // Google Calendar integration was removed from the app; drop the now-orphaned
      // tables for existing installs (idempotent no-op on subsequent boots and for
      // brand-new installs that never had them — see docs/RECREATION_SPEC.md §3.4.5).
      this.db.exec("DROP TABLE IF EXISTS google_calendar_tokens");
      this.db.exec("DROP TABLE IF EXISTS google_calendars");
      this.db.exec("DROP TABLE IF EXISTS calendar_events");
