...
const COMPONENT_NAME = "ektoswhispr";
...
class KDEShortcutManager {
  ...
  _findCallbackByFriendlyName(name) {
    ...
    for (const slotName of this.registeredSlots) {
      friendlyToSlot[`EktosWhispr ${slotName}`] = slotName;
      friendlyToSlot[`EktosWhispr`] = "dictation"; // legacy compat
    }
  ...
  async registerKeybinding(electronHotkey, slotName = "dictation", callback) {
    ...
    const actionId = [COMPONENT_NAME, slotName, "EktosWhispr", `EktosWhispr ${slotName}`];
