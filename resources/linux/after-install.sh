#!/bin/bash
# Post-install script for EktosWhispr (deb/rpm)
...
CHROME_SANDBOX=$(dpkg -L ektos-whispr 2>/dev/null | grep chrome-sandbox || echo "")
if [ -z "$CHROME_SANDBOX" ]; then
  # Fallback: conventional electron-builder install path
  CHROME_SANDBOX="/opt/EktosWhispr/chrome-sandbox"
fi
